/* 
  ...
  2 获取用户对小程序所授予获取地址的权限状态 scope
    1 假设用户点击获取收货地址的提示框确定 authsetting scope.address
      scope值true 直接调用获取收货地址
    2 假设用户点击获取收货地址的提示框取消
      scope值 false
      1 诱导用户自己打开授权设置页面(wx.openSetting)当用户重新给与获取地址权限的时
      2 获取收货地址
    3 假设用户从来没有调用过收货地址的api
      scope undefined 直接调用获取收货地址
    4 把获取到的收货地址 存入到本地储存中
  3 页面加载完毕
    0 onLoad onShow
    1 获取本地存储中的地址数据
    2 把数据设置给data中的一个变量
  ...
  8 全选和反选
    ...
    5 把购物车数组和allChecked重新设置回data 把购物车重新设置会缓存中
  9 商品数量的编辑
...
Page({
  ...
  //  设置购物车状态同时重新计算底部工具栏的数据 全选总价格购买的数量
  setCart(cart) {
